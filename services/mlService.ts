...
export class MlService {
   ...
   */
  public async loadModel(modelId: string, forceReload = false): Promise<any> {
    // tf.GraphModel
   ...
   */
  public findModelForLanguagePair(
    sourceCode: string,
    targetCode: string,
   ...
   */
  private tokenize(text: string, modelId: string): any {
    // tf.Tensor
  ...
  private detokenize(
    ...
    try {
      ...
      const confidenceScore =
        this.calculateTranslationConfidence(firstBatchItem);

      // Generate alternative translations from top-k predictions
      const alternatives = this.generateAlternativeTranslations(
        firstBatchItem,
        modelId,
      );
      ...
      const translatedText = String.fromCharCode(
        ...firstBatchItem.map((code) =>
          Math.max(32, Math.min(126, Math.round(Math.abs(code)))),
        ),
      );

      console.log(
        `Translation confidence: ${confidenceScore.toFixed(3)}, alternatives: ${alternatives.length}`,
      );
  ...
  public async translateText(
    ...
    try {
      ...
      const detokenizationResult = this.detokenize(
        outputTensor,
        modelConfig.id,
      );
      ...
      const confidence =
        detokenizationResult.confidence !== undefined
          ? detokenizationResult.confidence
          : 0.75; // Default if not provided by model
