...
import {
  mlService,
  TranslationResult as MlServiceTranslationResult,
} from './mlService';
import {
...
class TranslationService {
  ...
  async translate(
    ...
    if (sourceLanguageInput === null) {
      ...
      const detectionResult =
        await languageDetectionService.detectLanguage(text);
      sourceLanguage = detectionResult.detectedLanguage;
      detectionConfidence = detectionResult.confidence;
      console.log(
        `Detected source language: ${sourceLanguage.name} (Confidence: ${detectionConfidence.toFixed(2)})`,
      );
      if (detectionResult.confidence < 0.6) {
        console.warn(
          `Low confidence in language detection (${detectionResult.confidence.toFixed(2)}). Proceeding with ${sourceLanguage.name}, but result might be inaccurate.`,
        );
      }
    } else {
    ...
    try {
      ...
      const modelId = this.getModelIdForLanguagePair(
        sourceLanguage.code,
        targetLanguage.code,
      );
      ...
      if (!modelId) {
        console.log(
          `No model available for ${sourceLanguage.code} to ${targetLanguage.code}, using fallback`,
        );
        ...
        translationHistoryService
          ...
          .catch((err) =>
            console.error(
              'Failed to add no model config mock translation to history:',
              err,
            ),
          );
        return mockResult;
      ...
      if (!modelStatus?.isDownloaded) {
        ...
        translationHistoryService
          ...
          .catch((err) =>
            console.error(
              'Failed to add unavailable model mock translation to history:',
              err,
            ),
          );
        return mockResult;
      ...
      const mlResult = await mlService.translateText(
        text,
        sourceLanguage.code,
        targetLanguage.code,
        modelId,
      ...
      translationHistoryService
        ...
        .catch((err) =>
          console.error('Failed to add ML translation to history:', err),
        );
      return mlResult;
    } catch (error) {
      ...
      if (this.fallbackToMock) {
        ...
        translationHistoryService
          ...
          .catch((err) =>
            console.error('Failed to add mock translation to history:', err),
          );
        return mockResult;
      }
  ...
  private mockTranslate(
    ...
    if (
      ...
      mockTranslations[lowerText][targetLanguage.code]
    ) {
   ...
   */
  private getModelIdForLanguagePair(
    sourceCode: string,
    targetCode: string,
  ): string | undefined {
    ...
    const exactMatch = MODEL_CONFIGS.find(
      (config) =>
        config.srcLang === sourceCode && config.targetLang === targetCode,
    );
  ...
  getAvailableLanguagePairs(): {
    ...
    return MODEL_CONFIGS.map((config) => ({
      ...
      status: this.modelStatus.get(config.id) || {
        isDownloaded: false,
        isDownloading: false,
      },
    }));
